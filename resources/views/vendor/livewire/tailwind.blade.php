        ...
        <nav role="navigation" aria-label="Pagination Navigation" class="flex flex-col items-center gap-4 sm:flex-row sm:justify-between">
            ...
            <div class="flex items-center gap-1">
                ...
                @if ($paginator->onFirstPage())
                    <span class="inline-flex h-9 w-9 items-center justify-center rounded-lg text-gray-300 cursor-default">
                        ...
                        @foreach ($element as $page => $url)
                            <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                @if ($page == $paginator->currentPage())
                                    <span class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-gray-900 text-sm font-medium text-white">
                                        {{ $page }}
                                    </span>
                                @else
                                    <button
